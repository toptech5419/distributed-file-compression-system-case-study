...
> TCP sockets, a work-stealing-free task pool built on `std::future`, two interchangeable
> compression algorithms behind a polymorphic interface, and 14 test executables.
...
> Publishing a working solution would let it be copied. This is a public case study: architecture,
> design decisions, and test strategy — no solution code. Happy to walk through the implementation
> in an interview, or share it privately on request.
...
src/                          implementations + ClientMain / ServerMain
...
- **Chose `std::future` as the task-result channel over a shared results structure with a mutex.**
  ...
  synchronisation is the standard library's problem rather than mine. It also means an exception
  thrown inside a worker propagates to `.get()` instead of being swallowed on a detached thread.
...
- **Chose a factory returning the abstract base over branching on algorithm type at each call site.**
  `CompressionAlgorithm` declares `compress`/`decompress` as pure virtual over `std::vector<char>`.
  `AlgorithmFactory` maps a runtime type to a concrete instance. Adding a third algorithm touches
  the factory and nothing else — the server, the client protocol, and every test that exercises
  polymorphism stay unchanged. This is what "extensible" meant in the brief, made concrete.
...
- **Chose a virtual destructor on the abstract base.**
  ...
  the kind of line whose absence is invisible until it is not.
...
| Scale & robustness | Large-file scalability, edge cases, additional coverage |
...
- The wire protocol is minimal — operation byte plus payload. It has no length-prefixed framing for
  multi-message sessions and no version field, so extending it would be a breaking change.
...
- `Config` is read at startup only; there is no reload without a restart.
